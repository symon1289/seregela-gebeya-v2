          ...
          ordersData?.pages
            ...
            .map((order) => (
              <div key={order.id} className="bg-white p-4 rounded shadow">
                  ...
                  <div className="flex  space-x-2">
                    <div className="flex items-center space-x-2 py-2">
                      ...
                      {order.status === "successful" && (
                        <>
                          ...
                          >
                            <FaCheck className="text-green-800" size={10} />
                            <p> {order.status}</p>
                          </span>
                        </>
                      )}
                      ...
                      {order.status === "payment-rejected" && (
                        <>
                          ...
                          >
                            <FaBan className="text-red-800" size={10} />
                            <p> {order.status}</p>
                          </span>
                        </>
                      )}
                      {order.status === "canceled" && (
                        <>
                          <span
                            className={`px-3 py-1 text-xs justify-between rounded bg-red-200 text-red-800 flex items-center space-x-2`}
                          >
                            <FaXmark className="text-red-800" size={10} />
                            <p> {order.status}</p>
                          </span>
                        </>
                      )}
                    ...
                    {order.status === "successful" && (
                      <div className="flex justify-start items-center">
                        <FaDownload className="text-blue-600 mr-2" size={15} />
                ...
                {expandedOrderId === order.id && <OrderDetail id={order.id} />}
                <div className="flex justify-between items-center ">
                  ...
                  <p className="text-sm text-gray-700">
                    Delivery Method: {order?.delivery_type?.name}
                  </p>
                  <PaymentMethodDisplay paymentMethod={order?.payment_method} />
                </div>
                <div className="mt-2">
                  {order.status === "pending-delivery" && (
                    <span className="flex items-center space-x-2 bg-purple-100 px-3 py-2 rounded-lg">
                      ...
                      <p className="text-sm text-purple-600">
                        {DeliveryDelayCal(order?.delivered_at)}
                      </p>
                    </span>
                  )}
                  {order.status === "successful" && (
                    <span className="flex items-center space-x-2 bg-green-100 px-3 py-2 rounded-lg">
                      <FaCircleCheck className="text-green-600" size={15} />
                  ...
                  {order.status === "canceled" && (
                    <span className="flex items-center space-x-2 bg-red-100 px-3 py-2 rounded-lg">
                      <FaCircleXmark className="text-red-600" size={15} />
