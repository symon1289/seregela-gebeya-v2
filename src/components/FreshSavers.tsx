  ...
  useEffect(() => {
    const fetchFreshSavers = async () => {
      ...
      try {
        ...
        const fetchedProducts: Product[] = data.data.map((item: any) => ({
          id: item.id,
          name: item.name,
        ...
        if (page === 1) {
          setFreshSavers(fetchedProducts.slice(0, initialItemsToLoad));
        } else {
          setFreshSavers((prevSavers) => [...prevSavers, ...fetchedProducts]);
