...
const PriceFilter: React.FC<PriceFilterProps> = ({
  ...
  const updateSliderProgress = () => {
    if (minRangeRef.current && maxRangeRef.current) {
      ...
      const percent1 = ((minVal - ABSOLUTE_MIN) / (ABSOLUTE_MAX - ABSOLUTE_MIN)) * 100;
      const percent2 = ((maxVal - ABSOLUTE_MIN) / (ABSOLUTE_MAX - ABSOLUTE_MIN)) * 100;
      
      minRangeRef.current.style.background = 
        `linear-gradient(to right, #E5E7EB ${percent1}%, #3B82F6 ${percent1}%, #3B82F6 ${percent2}%, #E5E7EB ${percent2}%)`;
    }
  };
  ...
  const handleMaxChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const numValue = value === '' ? ABSOLUTE_MAX : Math.min(ABSOLUTE_MAX, parseInt(value));
  ...
  const handleMaxBlur = () => {
    const numValue = localMax === '' ? ABSOLUTE_MAX : Math.min(ABSOLUTE_MAX, parseInt(localMax));
    if (isNaN(numValue)) {
      setLocalMax(maxPrice.toString());
