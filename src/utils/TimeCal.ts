...
export const DeliveryDelayCal = (
    days_left_for_delivery: number,
    t: (key: string) => string // Accept `t` as an argument
): string => {
    ...
    if (days_left_for_delivery > 0) {
        return `${t("expected_delivery_on")} ${formattedDate}`;
    } else if (days_left_for_delivery === 0) {
        return `${t("delivery_expected_today")}`;
    } else {
        const delayDays = Math.abs(days_left_for_delivery);
        return delayDays === 1
            ? `${t("delayed_by_1_day")} ${formattedDate}`
            : `${t("delayed_by")} ${delayDays} ${t(
                  "days_since"
              )}  ${formattedDate}`;
    }
};
