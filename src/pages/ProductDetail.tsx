...
const ProductDetail: React.FC = () => {
  ...
  const fetchRelatedProducts = useCallback(
    async (subcategoryId: number, page: number, append: boolean = false) => {
      try {
        ...
        setHasMoreProducts(data.meta?.current_page < data.meta?.last_page);
        ...
        setRelatedError(
          err instanceof Error ? err.message : t("error_fetch_related_products")
        );
      } finally {
    ...
    return (
      <div className="flex items-center">
        ...
        <div className="flex flex-col text-xs text-gray-500">
          <span className="product-detail-decimal-part">.{decimalPart}</span>
          <span className="product-detail-birr-part">{t("birr")}</span>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-screen-xl mx-auto px-4 py-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
          ...
          <div className="bg-white rounded-lg py-4 shadow-sm  space-y-4">
            <div className="relative aspect-square">
              ...
              >
                <img
                  src={
                    product.image_paths?.[selectedImageIndex] || defaultImage
                  }
                  alt={product.name}
          ...
          <div className="bg-white rounded-lg py-6 shadow-sm">
            ...
            {product.name_am && (
              <h2 className="text-xl text-gray-600 mb-4">{product.name_am}</h2>
            )}
            ...
            {product.description && (
              <div className="mb-6">
                ...
                {product.description_am && (
                  <p className="text-gray-600 mt-2">{product.description_am}</p>
                )}
              </div>
            )}
            ...
            <div className="mb-6">
              ...
              <div className="flex items-center gap-4 mb-2">
                ...
                <span className="font-medium">{product.measurement_type}</span>
              </div>
              <div className="flex items-center gap-4 mb-2">
              ...
              <div className="flex items-center gap-4">
                ...
                <span className="font-medium">
                  {/* Stock Warning */}
                  {product.left_in_stock > 0 && product.left_in_stock <= 10 && (
                    <div className="text-red-500 text-sm animate-pulse">
                      {t("only")} {product.left_in_stock} {t("left")}
                    </div>
                  )}
                  {product.left_in_stock > 10 && <div>{t("in_stock")}</div>}
                    ...
                    <div className="absolute z-10 w-full mt-1 bg-white border rounded-lg shadow-lg max-h-48 overflow-y-auto">
                      {Array.from(
                        ...
                        <button
                          key={num}
                          onClick={() => {
                            handleQuantityChange(num.toString());
                            setIsDropdownOpen(false);
                          }}
                          className={`w-full px-3 py-2 text-left hover:bg-gray-50 ${
                            quantity === num ? "bg-gray-100" : ""
                          }`}
                        >
                          {num}
                        </button>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
                  {relatedProducts.map((relatedProduct) => (
                    <ProductCard
                      ...
                      image={relatedProduct.image_paths?.[0] || defaultImage}
                      originalPrice={String(relatedProduct.price)}
                      left_in_stock={relatedProduct.left_in_stock}
                    />
                  ))}
                </div>
              </div>
                ...
                <div className="relative order-1 md:order-2 flex-1 h-full flex items-center justify-center">
                  <div
                    className={`relative w-full h-full flex items-center justify-center overflow-hidden ${
                      isModalImageZoomed ? "cursor-zoom-out" : "cursor-zoom-in"
                    }`}
                    onClick={() => setIsModalImageZoomed(!isModalImageZoomed)}
                    onMouseMove={(e) => {
                  ...
                  >
                    <div className="relative w-full h-full flex items-center justify-center">
                      <img
                        src={product.image_paths?.[selectedImageIndex]}
                        alt={product.name}
                        className="max-h-full max-w-full object-contain"
                        style={{
                          transform: isModalImageZoomed
                            ? "scale(2)"
                            : "scale(1)",
                  ...
                  <button
                    ...
                    disabled={product.image_paths?.length === 1}
