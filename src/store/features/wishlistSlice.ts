...
export interface WishlistItem extends Pick<Product, 'id' | 'name' | 'price' | 'image_paths' | 'left_in_stock'> {
  id: number;
  price: string;
  image_paths: string[];
  quantity: number;
...
const loadState = (): WishlistState => {
  try {
    ...
    return { items: Array.isArray(parsedState.items) ? parsedState.items : [] };
  } catch (err) {
    console.error(err);
    return { items: [] };
  }
